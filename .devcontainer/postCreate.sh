...
echo
echo "== Next steps =="
echo "  1) lab/.env was just created from lab/.env.sample. Everything in it is pre-filled"
echo "     for the course environment and needs no editing - there is NO key to paste."
echo "  2) Model calls are KEYLESS: they reuse the sign-in from Module 3's first step"
echo "     (python m3_login.py), the same session that reads the claims book. Running"
echo "     against your own tenant instead? Put your own resource key or an Entra token"
echo "     in FOUNDRY_KEY - lab/.env.sample shows how."
